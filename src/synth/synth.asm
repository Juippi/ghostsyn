...
trigger_loop:
        ...
        jz no_trig_osc
        cmp al, 0x01
        jz no_trig_osc          ; note off should not set osc pitch

        ;; trigger oscillator (set pitch)
        and ebx, 0xfffffffc
        add ebx, esi
        fst dword [ebx + OSC_PARAM_ADD]
        jmp trig_done
no_trig_osc:
        ...
        ;; trigger envelope attack or decay
        and ebx, 0xfffffffc
        add ebx, esi
%ifdef ENABLE_ENVELOPE
        cmp al, 0x01
        jz .trigger_env_decay
        mov dword [ebx + ENV_PARAM_STAGE], ENV_STAGE_ATTACK
        jmp .trigger_env_done
.trigger_env_decay:
        mov dword [ebx + ENV_PARAM_STAGE], ENV_STAGE_RELEASE
.trigger_env_done:
%endif ;; ENABLE_ENVELOPE
